...
We have already discussed how can you run a test for a *continuous* random variable (such as wight, distance, glucose level, etc.). But what if your variable of interest is not continuos but rather discreet (for example, ratio of success for a new drug, the exam score, etc.)? 
...
According to the paper, exam is passed is the score is greater than 9, wo we are going to create a new variable with binary outcome `pass`/`fail`:
...
Now, the variable `final_score` is actually falling under **Binomial distribution**[^4]. We have (kind of) looked already how to deal with binomial distributed variables under the null hypothesis testing framework in this **intuitive example**[^5], but let's dig into more details. 
...
Now say we want to compare propotion for multiple groups rather than just one. For this purpose one can use **$\chi^2$ test for independence**. In general form, under the chi-square test we have following hypotheses:
...
This is also may be a tough task to calculate by hand that's why we usually rely on software.
...
As we can see, the pairs of `fail & 2`, `fail & 4` and `pass & 4` have the highest percentage of contribution (or in other words, they is some association).

## Summary

This was a brief overview of how we can perform hypothesis testings when we deal with discreet variables. In the next (and I hope the final) part I will finally introduce a concept of test power and effect size and discuss why p-value alone is not sufficient for decision making.
